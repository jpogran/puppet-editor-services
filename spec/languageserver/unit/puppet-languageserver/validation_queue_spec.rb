  ...
  describe '#enqueue' do
    shared_examples_for "single document which sends validation results" do |file_uri, file_content, validation_result|
      it 'should send validation results' do
        ...
        expect(PuppetLanguageServer::ValidationQueue).to receive(:send_diagnostics).with(client_handler_id, file_uri, validation_result)

        subject.enqueue(file_uri, document_version, client_handler_id)
    ...
    context 'for an invalid or missing documents' do
      it 'should not return validation results' do
        ...
        subject.enqueue(MANIFEST_FILENAME, document_version + 1, client_handler_id)
        # Wait for the thread to complete
        subject.drain_queue
      end
    end
    ...
    context 'for a multiple items in the queue' do
      ...
      it 'should only return the most recent validation results' do
        ...
        subject.reset_queue([
          { 'file_uri' => MANIFEST_FILENAME,   'doc_version' => document_version + 0, 'document_type' => :manifest,   'client_handler_id' => client_handler_id },
          { 'file_uri' => MANIFEST_FILENAME,   'doc_version' => document_version + 1, 'document_type' => :manifest,   'client_handler_id' => client_handler_id },
          { 'file_uri' => MANIFEST_FILENAME,   'doc_version' => document_version + 3, 'document_type' => :manifest,   'client_handler_id' => client_handler_id },
          { 'file_uri' => EPP_FILENAME,        'doc_version' => document_version + 1, 'document_type' => :epp,        'client_handler_id' => client_handler_id },
          { 'file_uri' => PUPPETFILE_FILENAME, 'doc_version' => document_version + 1, 'document_type' => :puppetfile, 'client_handler_id' => client_handler_id },
        ])
        ...
        expect(PuppetLanguageServer::ValidationQueue).to receive(:send_diagnostics).with(client_handler_id, MANIFEST_FILENAME, validation_result)
        expect(PuppetLanguageServer::ValidationQueue).to receive(:send_diagnostics).with(client_handler_id, EPP_FILENAME, validation_result)
        expect(PuppetLanguageServer::ValidationQueue).to receive(:send_diagnostics).with(client_handler_id, PUPPETFILE_FILENAME, validation_result)
        ...
        subject.enqueue(MANIFEST_FILENAME, document_version + 2, client_handler_id)
        # Wait for the thread to complete
        subject.drain_queue
      end
    end
  ...
  describe '#validate_sync' do
    shared_examples_for "document which sends validation results" do |file_uri, file_content, validation_result|
      it 'should send validation results' do
        ...
        subject.validate_sync(file_uri, document_version, client_handler_id, )
      end
    end
    ...
    it 'should not send validation results for documents that do not exist' do
      ...
      subject.validate_sync(MISSING_FILENAME, 1, client_handler_id)
    end
